<template>
  ...
  <v-container>
      <validation-observer
        ...
        v-slot="{ invalid }"
      >
      <div style="padding: 10px 0px;background-color: lightgray; font-size: 20px; text-align: center; "><b>Return</b></div><br/>
      <v-row>
...
<script>
  ...
  export default {
    ...
    apollo: {
      ...
      orders() {
        return {
          query: require('@/graphql/orders').default,
          pollInterval: false,
          skip: true,
          variables() {
            let filters = {
          ...
          update(data) {
            ...
            //   }).sort((a, b) => a.text.localeCompare(b.text))
            console.log(data)
            return data.orders.items.map(or=>{
              return {text:`#${or.internalOrderId}`,value:or.internalOrderId};
            })
          },
          error(error) {
            this.$store.commit('setError', {
    ...
    watch:{
      userId:{
        handler(newUserId) {
          ...
          newUserId && this.$apollo.queries.orders.start();
        }
      }
    },
    computed:{
